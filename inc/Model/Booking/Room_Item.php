...
class Room_Item extends Item {
	...
	public function get_start_date() {
		return apply_filters( $this->prefix( 'get_start_date' ), $this['check_in'], $this );
	}
	...
	public function get_end_date() {
		return apply_filters( $this->prefix( 'get_end_date' ), $this['check_out'], $this );
	}
	...
	public function get_timespan() {
		$timespan = U::rescue( function () {
			return new Timespan( $this->get_start_date(), $this->get_end_date() );
		});

		return apply_filters( $this->prefix( 'get_timespan' ), $timespan, $this );
	}
	 ...
	 *
	 * @param  \AweBooking\Model\Common\Timespan $timespan The new stay.
	 * @return boolean|WP_Error
	 */
	public function modify_stay( Timespan $timespan ) {
		...
		$this->attributes['check_in'] = $timespan->get_start_date()->toDateString();
		$this->attributes['check_out'] = $timespan->get_end_date()->toDateString();
	 ...
	 *
	 * @param  \AweBooking\Model\Common\Timespan $other_stay Other stay date.
	 * @return boolean
	 */
	public function can_change_stay( Timespan $other_stay ) {
		if ( ! $this->exists() ) {
			return false;
		}
		...
		foreach ( $diff as $piece ) {
			...
			if ( ! $checker->is_available_for( $room_unit, Timespan::from_period( $piece ) ) ) {
				return false;
			}
		}
	...
	public function apply_calendar_state() {
		...
		try {
			$state_event = new State_Event( $resource, $this->get_start_date(), $this->get_end_date(), $booking->get_state_status() );
			$booking_event = new Booking_Event( $resource, $this->get_start_date(), $this->get_end_date(), $booking );
		} catch ( \Exception $e ) {
			return false;
		}
