...
use AweBooking\Vendor\Faker\Generator as Faker;
...
class FactoryBuilder
{
     ...
     * @return void
     */
    public function __construct($class, array $definitions, array $states, array $afterMaking, array $afterCreating, Faker $faker)
    {
        $this->class = $class;
        $this->faker = $faker;
        $this->states = $states;
     ...
     *
     * @param  array  $attributes
     * @return \AweBooking\Vendor\Illuminate\Database\Eloquent\Collection|\AweBooking\Vendor\Illuminate\Database\Eloquent\Model|mixed
     */
    public function create(array $attributes = [])
    {
        $results = $this->make($attributes);
        if ($results instanceof Model) {
            $this->store(\AweBooking\Vendor\collect([$results]));
            $this->callAfterCreating(\AweBooking\Vendor\collect([$results]));
        } else {
            $this->store($results);
     ...
     * @return \AweBooking\Vendor\Illuminate\Database\Eloquent\Collection|mixed
     */
     ...
     *
     * @param \AweBooking\Vendor\Illuminate\Support\Collection  $results
     * @return void
     */
    ...
    {
        if ($this->amount === null) {
            return \AweBooking\Vendor\tap($this->makeInstance($attributes), function ($instance) {
                $this->callAfterMaking(\AweBooking\Vendor\collect([$instance]));
            });
        }
        if ($this->amount < 1) {
    ...
    protected function getRawAttributes(array $attributes = [])
    {
        ...
        $definition = \call_user_func($this->definitions[$this->class], $this->faker, $attributes);
     ...
     *
     * @param \AweBooking\Vendor\Illuminate\Support\Collection  $models
     * @return void
     */
