...
use AweBooking\Vendor\Carbon\CarbonInterface;
use DateTimeInterface;
use AweBooking\Vendor\Illuminate\Contracts\Database\Eloquent\Castable;
use AweBooking\Vendor\Illuminate\Contracts\Database\Eloquent\CastsInboundAttributes;
use AweBooking\Vendor\Illuminate\Contracts\Support\Arrayable;
use AweBooking\Vendor\Illuminate\Database\Eloquent\JsonEncodingException;
use AweBooking\Vendor\Illuminate\Database\Eloquent\Relations\Relation;
use AweBooking\Vendor\Illuminate\Support\Arr;
use AweBooking\Vendor\Illuminate\Support\Carbon;
use AweBooking\Vendor\Illuminate\Support\Collection as BaseCollection;
use AweBooking\Vendor\Illuminate\Support\Facades\Date;
...
trait HasAttributes
{
    ...
    public function getRelationValue($key)
    {
        ...
        if (\method_exists($this, $key) || (static::$relationResolvers[\get_class($this)][$key] ?? null)) {
    ...
    protected function getRelationshipFromMethod($method)
    {
        ...
        return \AweBooking\Vendor\tap($relation->getResults(), function ($results) use($method) {
    ...
    {
        $value = $this->isClassCastable($key) ? $this->getClassCastableAttributeValue($key, $value) : $this->mutateAttribute($key, $value);
    ...
    {
        return \AweBooking\Vendor\tap($this->getArrayAttributeByKey($key), function (&$array) use($path, $value) {
     ...
     *
     * @param  mixed  $value
     * @return \AweBooking\Vendor\Illuminate\Support\Carbon
     */
    ...
    {
        return \array_key_exists($key, $this->getCasts()) && \class_exists($class = $this->parseCasterClass($this->getCasts()[$key])) && !\in_array($class, static::$primitiveCastTypes);
    }
    /**
    ...
    protected function getOriginalWithoutRewindingModel($key = null, $default = null)
    {
        ...
        return \AweBooking\Vendor\collect($this->original)->mapWithKeys(function ($value, $key) {
    ...
    {
        static::$mutatorCache[$class] = \AweBooking\Vendor\collect(static::getMutatorMethods($class))->map(function ($match) {
