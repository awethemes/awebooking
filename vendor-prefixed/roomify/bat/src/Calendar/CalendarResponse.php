 ...
 */
namespace AweBooking\Vendor\Roomify\Bat\Calendar;

use AweBooking\Vendor\Roomify\Bat\Unit\UnitInterface;
use AweBooking\Vendor\Roomify\Bat\Constraint\Constraint;
/**
...
class CalendarResponse
{
    ...
    {
        $this->included_set[$unit->getUnitId()] = array('unit' => $unit, 'reason' => $reason);
    }
    /**
     * @param $unit
     * @param $reason
    ...
    {
        $this->excluded_set[$unit->getUnitId()] = array('unit' => $unit, 'reason' => $reason);
        if ($constraint !== NULL) {
