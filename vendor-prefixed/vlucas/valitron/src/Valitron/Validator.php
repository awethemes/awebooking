...
namespace AweBooking\Vendor\Valitron;
...
class Validator
{
     ...
     */
    public function setPrependLabels($prepend_labels = \true)
    {
    ...
    protected function validateRequired($field, $value, $params = array())
    {
        if (isset($params[0]) && (bool) $params[0]) {
            $find = $this->getPart($this->_fields, \explode('.', $field), \true);
            return $find[1];
        }
        if (\is_null($value) || \is_string($value) && \trim($value) === '') {
            return \false;
        }
        return \true;
    }
    ...
    {
        $acceptable = array('yes', 'on', 1, '1', \true);
        return $this->validateRequired($field, $value) && \in_array($value, $acceptable, \true);
    }
    /**
    ...
    protected function validateAscii($field, $value)
    {
        ...
        if (\function_exists('mb_detect_encoding')) {
            return \mb_detect_encoding($value, 'ASCII', \true);
        }
        // fallback with regex
    ...
    protected function validateCreditCard($field, $value, $params)
    {
        ...
        if ($numberIsValid()) {
            if (!isset($cards)) {
                ...
                $cardRegex = array('visa' => '#^4[0-9]{12}(?:[0-9]{3})?$#', 'mastercard' => '#^(5[1-5]|2[2-7])[0-9]{14}$#', 'amex' => '#^3[47][0-9]{13}$#', 'dinersclub' => '#^3(?:0[0-5]|[68][0-9])[0-9]{11}$#', 'discover' => '#^6(?:011|5[0-9]{2})[0-9]{12}$#');
                if (isset($cardType)) {
    ...
    protected function validateRequiredWith($field, $value, $params, $fields)
    {
        ...
        if (isset($params[0])) {
            ...
            foreach ($reqParams as $requiredField) {
                ...
                if (isset($fields[$requiredField]) && !\is_null($fields[$requiredField]) && (\is_string($fields[$requiredField]) ? \trim($fields[$requiredField]) !== '' : \true)) {
                    if (!$allRequired) {
                        $conditionallyReq = \true;
                        break;
                    } else {
                        $emptyFields++;
                    }
        ...
        if ($conditionallyReq && (\is_null($value) || \is_string($value) && \trim($value) === '')) {
            return \false;
        }
        return \true;
    }
    ...
    protected function validateRequiredWithout($field, $value, $params, $fields)
    {
        ...
        if (isset($params[0])) {
            ...
            foreach ($reqParams as $requiredField) {
                ...
                if (!isset($fields[$requiredField]) || (\is_null($fields[$requiredField]) || \is_string($fields[$requiredField]) && \trim($fields[$requiredField]) === '')) {
                    if (!$allEmpty) {
                        $conditionallyReq = \true;
                        break;
                    } else {
     ...
     * @param bool $stop
     */
    public function stopOnFirstFail($stop = \true)
    {
    ...
    public function rule($rule, $fields)
    {
        ...
        if (!isset($errors[$rule])) {
            ...
            if (!\method_exists($this, $ruleMethod)) {
                throw new \InvalidArgumentException("Rule '" . $rule . "' has not been registered with " . \get_called_class() . "::addRule().");
            }
        }
        ...
        $this->_validations[] = array('rule' => $rule, 'fields' => (array) $fields, 'params' => (array) $params, 'message' => $message);
        return $this;
    }
    /**
     * Add label to rule
    ...
    protected function checkAndSetLabel($field, $message, $params)
    {
        if (isset($this->_labels[$field])) {
            ...
            $message = $this->prepend_labels ? \str_replace('{field}', \ucwords(\str_replace('_', ' ', $field)), $message) : \str_replace('{field} ', '', $message);
        }
        return $message;
    }
    /**
